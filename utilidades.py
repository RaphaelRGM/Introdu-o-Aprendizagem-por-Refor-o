...
def plot_optimal_policy_return(num_states, optimal_policy, rewards, V):
    ...
    ax.set_facecolor('#282a36ff')
    fig.patch.set_facecolor('#282a36ff')
    ...
    ax.set_title("Política ideal", fontsize=18, color="white")
...
def plot_q_values(num_states, q_left_star, q_right_star, rewards):
    ...
    for i in range(num_states):
        ax.text(i+0.2, 0.6, round(q_left_star[i], 2),
                fontsize=16, ha="center", va="center", color="yellow")
        ax.text(i+0.8, 0.6, round(q_right_star[i], 2),
                fontsize=16, ha="center", va="center", color="lightgreen")
    ...
    ax.set_yticklabels([])
    ax.tick_params(axis='both', which='both', length=0)
    ax.set_title("Q(s,a)", fontsize=18, color="white")
